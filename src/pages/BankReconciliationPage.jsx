...
const OperationStatusModal = ({ state, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 px-4">
      <div className={`w-full max-w-md rounded-3xl border p-6 shadow-2xl ${tone.panel}`}>
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {state.details ? (
              <p className="mt-2 text-xs font-medium text-slate-500">{state.details}</p>
            ) : null}
          </div>
        </div>
...
export default function BankReconciliationPage() {
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setOperationStatus({
        ...
        details: importedCount ? `${importedCount} transaction row${importedCount === 1 ? "" : "s"} imported.` : "",
      });
      toast.success(response?.message || "Bank statement uploaded successfully");
    },
    onError: (error) => {
      setOperationStatus({
        open: true,
        ...
        variant: "error",
        title: "Upload failed",
        message: error?.response?.data?.message || error?.message || "Bank statement upload failed.",
  ...
  const autoMutation = useMutation({
    ...
    onSuccess: (response) => {
      invalidate();
      const exactCount = response?.data?.summary?.exactMatchCount ?? response?.data?.exactMatches?.length ?? 0;
      const potentialCount =
        response?.data?.summary?.potentialMatchCount ?? response?.data?.potentialMatches?.length ?? 0;
      const noMatches = response?.data?.noMatches ?? (exactCount === 0 && potentialCount === 0);
      ...
      setOperationStatus({
        ...
        message: noMatches
          ? response?.message || "No matching transactions were found for the selected account."
      ...
      toast.success(`Auto reconciliation completed. Exact: ${exactCount}, Potential: ${potentialCount}`);
    },
    onMutate: () => {
      setOperationStatus({
        open: true,
    ...
    onError: (error) => {
      setOperationStatus({
        ...
        message: error?.response?.data?.message || error?.message || "Auto reconciliation could not be completed.",
        details: "Try again after reviewing the selected ledger and imported statement rows.",
      });
    },
  });
    ...
    if (overviewQuery.isFetching) {
      setOperationStatus((current) => {
        ...
        return {
          ...
          message: "Fetching bank transactions and book entries for the selected ledger.",
  ...
  useEffect(() => {
    if (!operationStatus.open || operationStatus.variant === "loading") return undefined;

    const timer = window.setTimeout(() => {
      setOperationStatus((current) => ({ ...current, open: false, source: null }));
    }, 2600);
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      if (!rows.length) {
        throw new Error("The selected file does not contain any transaction rows");
      }

      const transactions = rows.map((row, index) => {
        const transactionDate = parseUploadedDate(
          row.transactionDate || row.date || row.Date || row["Transaction Date"]
        );
        const valueDate = parseUploadedDate(row.valueDate || row["Value Date"]);

        if (!transactionDate) {
          throw new Error(`Invalid transaction date in uploaded file at row ${index + 2}`);
        }
  ...
  return (
    <div className="mx-auto w-full max-w-[1900px] space-y-6 px-4 pb-6 xl:px-6 2xl:px-8">
      <OperationStatusModal
        ...
        onClose={() => setOperationStatus((current) => ({ ...current, open: false, source: null }))}
      />
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-black text-slate-900">{fmtCurrency(Math.abs(brs.bankBalance || 0))}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold text-slate-500">Book Balance</p>
          <p className="mt-2 text-2xl font-black text-slate-900">{fmtCurrency(Math.abs(brs.bookBalance || 0))}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold text-slate-500">Difference</p>
          <p className="mt-2 text-2xl font-black text-slate-900">{fmtCurrency(Math.abs(brs.difference || 0))}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <table className="min-w-[640px] divide-y divide-slate-100 text-[12px]">
              <thead className="sticky top-0 bg-slate-50">
                <tr>
                  {["Date", "Amount", "Reference", "Type", "Status"].map((label) => (
                    <th key={label} className="px-3 py-2.5 text-left text-[11px] font-semibold text-slate-500">
                      {label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-50">
                {bankTransactions.map((bankTransaction) => (
                  ...
                  >
                    <td className="px-3 py-2.5 text-slate-600 whitespace-nowrap">{fmtDate(bankTransaction.transactionDate)}</td>
                    <td className="px-3 py-2.5 font-black text-slate-900 whitespace-nowrap">{fmtCurrency(bankTransaction.amount)}</td>
                    <td className="px-3 py-2.5 text-slate-600 max-w-[180px] truncate">{bankTransaction.reference || "—"}</td>
                    <td className="px-3 py-2.5 text-slate-600 whitespace-nowrap">{bankTransaction.type}</td>
                  ...
                  return (
                    <tr key={payment._id} className="hover:bg-slate-50">
                      <td className="px-3 py-2.5 min-w-[150px]">
                        ...
                        <div className="text-[11px] text-slate-500 line-clamp-2">{bookEntryDescription}</div>
                        {payment.journalNumber && (
                          <div className="mt-1 text-[11px] font-medium text-slate-400">{payment.journalNumber}</div>
                        )} 
                      </td>
                      <td className="px-3 py-2.5 text-slate-600 whitespace-nowrap">{fmtDate(bookEntryDate)}</td>
                            ...
                            journalLines.map((line) => (
                              ...
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <div className="text-[11px] font-semibold text-slate-800">{line.accountName || "Unknown Account"}</div>
                                    {/* <div className="text-xs text-slate-500">
                                      {line.accountCode || "—"}
                                      {line.isBankLedgerLine ? " • Selected bank ledger line" : ""}
                                    </div> */}
                                    {line.description ? (
                                      <div className="mt-1 text-[10px] text-slate-500 line-clamp-2">{line.description}</div>
                                    ) : null}
                                  </div>
                      ...
                      <td className="px-3 py-2.5 text-slate-600 whitespace-nowrap">{fmtCurrency(bookEntryTotalAmount)}</td>
                      <td className="px-3 py-2.5">
            ...
            {!selectedBank ? (
              ...
              <div className="mt-4 space-y-4">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <p className="mt-2 text-base font-black text-slate-900">{fmtCurrency(selectedBank.amount)}</p>
                  <p className="text-xs text-slate-500">Available: {fmtCurrency(availableSelectedBankAmount)}</p>
                </div>

                {selectedSuggestions.length > 0 && (
                  <div className="rounded-xl border border-blue-100 bg-blue-50 p-3">
                    <p className="text-xs font-semibold text-blue-700">Suggested Matches And Diagnostics</p>
                        ...
                        <div key={`${suggestion.paymentId}-${suggestion.matchType}`} className="rounded-lg border border-blue-100 bg-white p-2">
                          ...
                          {suggestion.reference || suggestion.amount ? (
                            <div className="mt-1 text-[11px] text-slate-500">
                              {suggestion.reference || "No ref"} • {fmtCurrency(suggestion.amount)} • {fmtDate(suggestion.date)}
                            </div>
                          ) : null}
                          {(suggestion.reasons || []).length > 0 ? (
                            <div className="mt-2 rounded-lg border border-emerald-100 bg-emerald-50 p-2">
                              <p className="text-[11px] font-semibold text-emerald-700">Why it matched</p>
                          ...
                          {(suggestion.failures || []).length > 0 ? (
                            <div className="mt-2 rounded-lg border border-amber-100 bg-amber-50 p-2">
                              <p className="text-[11px] font-semibold text-amber-700">Why it did not become exact</p>
                              <ul className="mt-1 list-disc pl-4 text-[11px] text-slate-600">
                                {suggestion.failures.slice(0, 4).map((failure) => (
                          ...
                          {(suggestion.sharedTokens || []).length > 0 ? (
                            <p className="mt-2 text-[11px] text-blue-700">
                              Shared tokens: {suggestion.sharedTokens.slice(0, 5).join(", ")}
                            </p>
                          ) : null}
                        </div>
                      ))}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-slate-900">{fmtCurrency(Math.abs(brs.bookBalance || 0))}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Bank Balance</span>
                <span className="font-bold text-slate-900">{fmtCurrency(Math.abs(brs.bankBalance || 0))}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Difference</span>
                <span className="font-bold text-slate-900">{fmtCurrency(Math.abs(brs.difference || 0))}</span>
              </div>
