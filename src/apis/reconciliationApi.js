...
export const getReconciliationOverviewApi = async (companyId, params = {}) => {
  const { data } = await API.get(`/bank-reconciliation/overview/${companyId}`, {
    params,
  });
  return data;
};
...
export const uploadBankStatementApi = async (payload) => {
  const { data } = await API.post("/bank-reconciliation/bank/import", payload);
  return data;
};

export const autoMatchBankTransactionsApi = async (companyId) => {
  const { data } = await API.post(`/bank-reconciliation/reconcile/auto/${companyId}`);
  return data;
};

export const manualMatchApi = async (payload) => {
  const { data } = await API.post("/bank-reconciliation/reconcile/manual", payload);
  return data;
};

export const unmatchApi = async (payload) => {
  const { data } = await API.post("/bank-reconciliation/reconcile/unlink", payload);
  return data;
};

export const createPaymentFromBankTransactionApi = async (payload) => {
  const { data } = await API.post("/bank-reconciliation/reconcile/create-payment", payload);
  return data;
};

export const getBankReconciliationStatementApi = async (companyId) => {
  const { data } = await API.get(`/bank-reconciliation/brs/${companyId}`);
  return data;
};
