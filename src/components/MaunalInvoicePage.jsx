...
const addMonths = (dateLike, months) => {
    const date = new Date(dateLike);
    if (Number.isNaN(date.getTime())) return null;
...
const buildRecurringTermSchedule = (po = {}) => {
    ...
    const startDate = new Date(po.poDate || po.referenceDate);
    const endDate = new Date(po.deliveryDate || po.dueDate || po.poDate || po.referenceDate);
    ...
    for (let index = 0; index < totalTerms; index += 1) {
        ...
        if (paymentTermType === "monthly") {
            ...
            termEnd = nextStart ? addDays(nextStart, -1) : new Date(endDate);
        } else {
            termEnd = addDays(cursor, 6);
        }

        if (!termEnd || termEnd > endDate) {
            termEnd = new Date(endDate);
        }
        ...
        terms.push({
            ...
            shortLabel: paymentTermType === "monthly" ? `Month ${index + 1}` : `Week ${index + 1}`,
        });

        if (paymentTermType === "monthly") {
            cursor = addMonths(cursor, 1) || new Date(endDate);
        } else {
            cursor = addDays(termEnd, 1) || new Date(endDate);
        }
    }
...
const getInvoiceRaisedAmount = (invoice = {}) => {
    ...
    return roundMoney(
        invoice.invoiceAmount || invoice.amountDue || invoice.totalAmount || 0,
    );
};
...
const getRaisedAmountForPoItem = (invoices = [], poItem = {}) => {
    ...
    return roundMoney(
        invoices.reduce((invoiceSum, linkedInvoice) => {
            ...
            const matchedItem = invoiceItems.filter((invoiceItem) =>
                getPoItemMatchKeys(invoiceItem).some((key) => matchKeys.has(key)),
            );

            return (
                invoiceSum +
                matchedItem.reduce(
                    (sum, invoiceItem) => sum + Number(invoiceItem.totalAmount || invoiceItem.total || 0),
                    0,
                )
            );
        }, 0),
    ...
    milestones.forEach((m, idx) => {
        ...
        const alreadyInvoiced = roundMoney(
            Math.max(
                ...
                orderedInvoices.reduce((sum, invoice) => {
                    const milestoneEntry = (invoice.milestones || []).find(
                        (entry) =>
                            String(entry?.milestoneId || "") === String(m?._id || "") ||
                            String(entry?.title || "").trim() === String(m?.title || "").trim(),
                    );

                    return sum + Number(milestoneEntry?.invoicedAmount || milestoneEntry?.amount || 0);
                }, 0),
            ),
        );
...
const ManualInvoicePage = () => {
    ...
    const handleSelectPO = async (po) => {
        ...
        const paymentTermType = getNormalizedPaymentTermType(selectedPO);
        let paymentTermSchedule = {
            totalInstallments: 1,
            currentInstallmentNo: 1,
            currentWindowStart: formatDateInput(selectedPO.poDate),
            currentWindowEnd: formatDateInput(selectedPO.deliveryDate || selectedPO.dueDate),
            label: "",
        };
        ...
        // ──────────────────────────────────────────────────────────
        if (paymentTermType === "milestone" && Array.isArray(selectedPO.milestones) && selectedPO.milestones.length > 0) {
            const rows = getMilestoneInvoiceRows(selectedPO);
            if (rows.length === 0) {
            ...
            paymentTermSchedule = {
                ...
                currentInstallmentNo: rows.find((row) => !row._isCarryForward)?._milestoneIndex + 1 || rows[0]?._milestoneIndex + 1 || 1,
                currentWindowStart: milestoneRowsUI.find((row) => !row.isCarryForward)?.dueDate || milestoneRowsUI[0]?.dueDate || formatDateInput(selectedPO.poDate),
                currentWindowEnd: milestoneRowsUI.find((row) => !row.isCarryForward)?.dueDate || milestoneRowsUI[0]?.dueDate || formatDateInput(selectedPO.deliveryDate || selectedPO.dueDate),
                label: `Milestone Based${rows.some((row) => row._isCarryForward) ? " + previous remaining" : ""}`,
            };
        }
    ...
    if (!recurringContext) {
        ...
        return;
            ...
            if (items.length > 0) {
                derivedItems = items
                    .map((item) => {
                        const itemTotal = Number(item.totalAmount || item.total || 0);
                        const itemDistributions = distributeAmountAcrossTerms(itemTotal, recurringContext.totalTerms);
                        const scheduledPrevious = roundMoney(
                            itemDistributions
                                .slice(0, recurringContext.currentTermIndex)
                                .reduce((sum, amount) => sum + amount, 0),
                        );
                        const actualPrevious = getRaisedAmountForPoItem(previousInvoices, item);
                        const carryForwardAmount = roundMoney(
                            Math.max(0, scheduledPrevious - actualPrevious),
                        );
                        const currentTermAmount = roundMoney(
                            recurringContext.currentTerm ? itemDistributions[recurringContext.currentTermIndex] || 0 : 0,
                        );
                        const lineAmount = roundMoney(carryForwardAmount + currentTermAmount);
                        ...
                        return {
                            ...
                            description: `${item.description || ""} (${recurringContext.currentTerm?.shortLabel || paymentTermType}${carryForwardAmount > 0 ? " + previous remaining" : ""})`,
                            hsnSac: item.hsnSac || item.hsnCode || "",
                            quantity: 1,
                            ...
                            totalManuallyEdited: false,
                            currentTermAmount,
                            previousCarryForward: carryForwardAmount,
                            remainingAfterInvoice: roundMoney(Math.max(0, itemTotal - actualPrevious - lineAmount)),
                        };
                    })
                    .filter(Boolean);
            } else {
                const lineAmount = roundMoney(
                    recurringContext.carryForwardAmount + recurringContext.currentTermAmount,
                );
                derivedItems = lineAmount > 0 ? [{
                    itemId: `${paymentTermType}-${recurringContext.currentTermNumber || recurringContext.totalTerms}`,
                    poItemId: "",
                    description: `Services - ${recurringContext.currentTerm?.label || "Previous Remaining"}${recurringContext.carryForwardAmount > 0 ? " + previous remaining" : ""}`,
                    hsnSac: "",
                    quantity: 1,
                    rate: lineAmount,
                    ...
                    combinedGstRate: 0,
                    totalManuallyEdited: false,
                    currentTermAmount: recurringContext.currentTermAmount,
                    previousCarryForward: recurringContext.carryForwardAmount,
                }] : [];
            }
            ...
            paymentTermSchedule = {
                totalInstallments: recurringContext.totalTerms,
                currentInstallmentNo: recurringContext.currentTermNumber,
                currentWindowStart: recurringContext.currentWindowStart,
                currentWindowEnd: recurringContext.currentWindowEnd,
                label: recurringContext.label,
            };
        }
        ...
        setInvoice((prev) => ({
            ...
            items: derivedItems,
            monthlyBillingInfo: ["monthly", "weekly"].includes(paymentTermType)
                ? {
                    paymentTermType,
                    currentTermNo: paymentTermSchedule.currentInstallmentNo,
                    totalTerms: paymentTermSchedule.totalInstallments,
                    periodLabel: paymentTermSchedule.label,
                    periodStart: paymentTermSchedule.currentWindowStart,
                    periodEnd: paymentTermSchedule.currentWindowEnd,
                }
                : undefined,
